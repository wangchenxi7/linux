...
delete_old_kernel_contents () {
	if [ $OS_DISTRO == "CentOS Linux" ]
	then
		echo "sudo rm /boot/initramfs-${version}${LocalVersion}   /boot/System.map-${version}${LocalVersion}  /boot/vmlinuz-${version}${LocalVersion} "
		sleep 1
		sudo rm /boot/initramfs-${version}${LocalVersion}   /boot/System.map-${version}${LocalVersion}  /boot/vmlinuz-${version}${LocalVersion}
	elif [ $OS_DISTRO == "Ubuntu" ]
	then
